...
<div class="py-12 bg-transparent min-h-screen" x-data="productDetail">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div x-show="!loading && product" class="mb-8" style="display: none;">
            <a href="{{ url('/products') }}" class="inline-flex items-center gap-2 text-xs font-bold text-brand-accent hover:text-brand-terracotta transition-colors">
        ...
        <div x-show="!loading && relatedProducts.length > 0" class="mt-20 border-t border-brand-beige pt-16" style="display: none;">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-8">
                <template x-for="rp in relatedProducts" :key="rp.id">
                    <a :href="'{{ url('/products') }}/' + rp.id" class="group bg-white border border-brand-beige/80 rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between min-h-[300px]">
                        <div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('productDetail', () => ({
            ...
            async init() {
                this.loadReviews();
                try {
                    const response = await fetch(`{{ url("/api/products") }}/${this.productId}`, {
                    ...
                    if (this.product && this.product.category_id) {
                        const relatedRes = await fetch(`{{ url("/api/products") }}?category_id=${this.product.category_id}`, {
                            headers: { 'X-API-KEY': 'craftive-public-key-2026' }
                        });
            ...
            async loadReviews() {
                try {
                    const response = await fetch(`{{ url("/api/products") }}/${this.productId}/reviews`, {
                        headers: { 'X-API-KEY': 'craftive-public-key-2026' }
                    });
                    if (response.ok) {
